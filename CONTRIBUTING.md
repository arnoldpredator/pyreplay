...
**Shipped 2026-08-01** — now catalog entry #112 in [FEATURES.md](FEATURES.md): `table` in the view select for uniform list-of-dicts (exact key-set equality) and same-length fully-visible tuples — per-cell diff highlight, 14-column cap announced, sortable headers that reorder DISPLAY only (true indices kept, note verbatim, windowed containers say "only the visible window sorts").
